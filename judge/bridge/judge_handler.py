...
class JudgeHandler(ZlibPacketHandler):
    ...
    def on_grading_end(self, packet):
        ...
        if submission.lesson_id:
            try:
                from judge.models.course import Enrollment, Lesson, LessonProgress
                lesson = Lesson.objects.select_related('chapter__course').get(id=submission.lesson_id)
                course_id = lesson.chapter.course_id

                # If submission is AC, check if lesson can be auto-completed
                is_lesson_completed = False
                if submission.result == 'AC' and lesson.are_required_problems_completed_by(submission.user.user):
