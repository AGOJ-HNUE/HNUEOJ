...
class ProblemSubmitMixin:
    ...
    def handle_submission_post(self, request):
        ...
        if not form.is_valid():
            if is_ajax:
                error_msg = '; '.join([f"{', '.join(e)}" for f, e in form.errors.items()]) if form.errors else str(_('Dữ liệu không hợp lệ.'))
                return JsonResponse({'success': False, 'error': error_msg}, status=400), form
        ...
        if (
            ...
            if is_ajax:
                return JsonResponse({'success': False, 'error': str(_('You submitted too many submissions.'))}, status=429), None
        ...
        if not self.object.allowed_languages.filter(id=form.cleaned_data['language'].id).exists():
            if is_ajax:
                return JsonResponse({'success': False, 'error': str(_('Ngôn ngữ nộp bài không hợp lệ.'))}, status=403), None
            raise PermissionDenied()

        if not request.user.is_superuser and self.object.banned_users.filter(id=request.profile.id).exists():
            if is_ajax:
                return JsonResponse({'success': False, 'error': str(_('You have been declared persona non grata for this problem.'))}, status=403), None
        ...
        if self.remaining_submission_count == 0:
            if is_ajax:
                return JsonResponse({'success': False, 'error': str(_('You have exceeded the submission limit for this problem.'))}, status=400), None
        ...
        if settings.VNOJ_ENABLE_ORGANIZATION_CREDIT_LIMITATION:
            ...
            if organization is not None:
                if not organization.has_credit_left():
                    ...
                    if is_ajax:
                        return JsonResponse({'success': False, 'error': str(_('The organization %s has no credit left.') % org_name)}, status=400), None
        ...
        if is_ajax:
            return JsonResponse({
                ...
                'status_url': reverse('submission_status', args=(new_submission.id,)),
            }), None
...
class ProblemSubmit(LoginRequiredMixin, ProblemMixin, TitleMixin, ProblemSubmitMixin, SingleObjectFormView):
    ...
    def form_invalid(self, form):
        if self.request.headers.get('x-requested-with') == 'XMLHttpRequest' or 'application/json' in self.request.META.get('HTTP_ACCEPT', ''):
            error_msg = '; '.join([f"{', '.join(e)}" for f, e in form.errors.items()]) if form.errors else str(_('Dữ liệu không hợp lệ.'))
